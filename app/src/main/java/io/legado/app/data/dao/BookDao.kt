...
interface BookDao {
    ...
    @Query(
        ...
        UPDATE books
        SET
            bookUrl = :newUrl || substr(bookUrl, length(:oldUrl) + 1),
            tocUrl = CASE
                WHEN tocUrl LIKE :oldUrl || '%'
                THEN :newUrl || substr(tocUrl, length(:oldUrl) + 1)
                ELSE tocUrl
            END
        WHERE origin = :oldUrl
        """
    )
    fun replaceBookUrls(oldUrl: String, newUrl: String): Int
}